...
Page({
  ...
  data: {
    //跳转链接
    skipLinks: '/pages/productSearchPage/productSearchPage',
    // 如果是从搜索页面跳转回来的 会带上搜索条件
    searchCondition: {},
    // 是否显示条件框
    whetherToDisplayAConditionBox: false,
    // 条件查询的标题
    conditionalQueryTitle: '按以下搜索条件查询到的所有产品',
  ...
  loadData(parameter = null, isPullDownRefresh = false, isReach = false) {
    ...
    return new Promise(function (resolve, reject) {
      if (app.globalData.isItAJumpFromTheSearchPage && !isPullDownRefresh && !isReach) {
        ...
        that.setData({
          productList: that.data.productList.concat(app.globalData.productList),
      ...
      filter_product(parameter, that).then(res => {
        ...
        for(let i = 0;i<res.data.length;i++){
          if (res.data[i]['成本估算'] == '暂无') {
            res.data[i]['成本估算'] == '暂无'
          }
          if (res.data[i]['成本估算'] != '暂无') {
            res.data[i]['成本估算'] = String(res.data[i]['成本估算'].toFixed(2)) + '元/对'
          }
        }
  ...
  callbackAfterDeletion(e) {
    let that = this
    var productInfo = e.detail.productInfo
    var index = that.data.productList.findIndex(item => {
      return item.unique_id === productInfo.unique_id
    })
    if (index !== -1) {
  ...
  onShow: function () {
    ...
    if (app.globalData.isItAJumpFromTheSearchPage) {
      ...
      // 代表是从其他页面跳回来的 需要把productList 还原为原来的数据
      that.data.productList = []
      var b = 1
