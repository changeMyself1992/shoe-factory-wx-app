...
Page({
  ...
  data: {
    //跳转链接 入库
    skipLinks: '/pages/warehouseImportAndExportSearchPage/warehouseImportAndExportSearchPage?type=import',
    // 如果是从搜索页面跳转回来的 会带上搜索条件
    searchCondition: {},
    // 是否显示条件框
    whetherToDisplayAConditionBox: false,
    // 条件查询的标题
    conditionalQueryTitle: '按以下搜索条件查询到的所有入库事件',
  ...
  handInStock(e) {
    let that = this
    wx.navigateTo({
      url: '/pages/warehouseHandMoveImportAndExport/warehouseHandMoveImportAndExport?type=import',
    })
  },
  // 采购单入库
  orderInStock(e) {
    let that = this
    wx.navigateTo({
      url: '/pages/warehousePurchaseOrderImport/warehousePurchaseOrderImport',
    })
  },
