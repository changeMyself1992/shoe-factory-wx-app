...
Page({
  ...
  data: {
    //跳转链接
    skipLinks: '/pages/clientSearchPage/clientSearchPage',
    // 如果是从搜索页面跳转回来的 会带上搜索条件
    searchCondition: {},
    // 是否显示条件框
    whetherToDisplayAConditionBox: false,
    // 条件查询的标题
    conditionalQueryTitle: '按以下搜索条件查询到的所有客户',
  ...
  loadData(parameter = null, isPullDownRefresh = false, isReach = false) {
    ...
    return new Promise(function (resolve, reject) {
      if (app.globalData.isItAJumpFromTheSearchPage && !isPullDownRefresh && !isReach) {
        ...
        that.setData({
          clientList: that.data.clientList.concat(app.globalData.clientList),
          searchCondition: app.globalData.searchCondition,
          whetherToDisplayAConditionBox: true
        });
        return resolve('加载客户单信息成功！！')
      ...
      filter_client(parameter, that).then(res => {
        ...
        if (!list || list.length == 0) {
          ...
          that.setData({
            clientList: that.data.clientList.concat(list),
          });
          resolve(res.msg)
        }
      })
  ...
  callbackAfterDeletion(e) {
    let that = this
    var clientInfo = e.detail.clientInfo
    var index = that.data.clientList.findIndex(item => {
      return item.unique_id === clientInfo.unique_id
    })
    if (index !== -1) {
      that.data.clientList.splice(index, 1)
      that.setData({
        clientList: that.data.clientList
      })
    }
  },
  ...
  onShow: async function () {
    ...
    if (app.globalData.isItAJumpFromTheSearchPage) {
      ...
      // 代表是从其他页面跳回来的 需要把clientList 还原为原来的数据
      that.data.clientList = []
      var b = 1
