...
Page({
  ...
  data: {
    //跳转链接
    skipLinks: '/pages/personnelListSearchPage/personnelListSearchPage',
    // 如果是从搜索页面跳转回来的 会带上搜索条件
    searchCondition: {},
    // 是否显示条件框
    whetherToDisplayAConditionBox: false,
    // 条件查询的标题
    conditionalQueryTitle: '按以下搜索条件查询到的所有员工',
    // 是否显示扫码图标
    whetherToShowScanIcon: false,
  ...
  loadData(parameter = null, isPullDownRefresh = false, isReach = false) {
    ...
    return new Promise(function(resolve, reject) {
      if (app.globalData.isItAJumpFromTheSearchPage && !isPullDownRefresh && !isReach) {
        ...
        that.setData({
          personnelList: that.data.personnelList.concat(app.globalData.staffListFromQuery),
      ...
      filter_user(parameter, that).then(res => {
        ...
        if (!list || list.length == 0) {
          ...
          that.setData({
            personnelList: that.data.personnelList.concat(list),
          });
          resolve(res.msg)
        }
      })
  ...
  callbackAfterDeletion(e) {
    ...
    var index = that.data.personnelList.findIndex(item => {
      return item.unique_id === employees.unique_id
    })
    if (index !== -1) {
      that.data.personnelList.splice(index, 1)
      that.setData({
        personnelList: that.data.personnelList
      })
    }
  },
  ...
  onShow: async function() {
    ...
    if (app.globalData.isItAJumpFromTheSearchPage) {
      ...
      // 代表是从其他页面跳回来的 需要把personnelList 还原为原来的数据
      that.data.personnelList = []
      var b = 1
