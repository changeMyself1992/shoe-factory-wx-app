// pages/supplierPriceList/supplierPriceList.js
const app = getApp();
import {
...
Page({
  ...
  data: {
    //跳转链接
    skipLinks: '/pages/supplierSearchPage/supplierSearchPage',
    // 如果是从搜索页面跳转回来的 会带上搜索条件
    searchCondition: {},
    // 是否显示条件框
    whetherToDisplayAConditionBox: false,
    // 条件查询的标题
    conditionalQueryTitle: '按以下搜索条件查询到的所有供应商报价单',
  ...
  loadData(parameter = null, isPullDownRefresh = false, isReach = false) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      //   that.setData({
      //     suplierList: that.data.suplierList.concat(app.globalData.suplierList),
      ...
      //     供应商unique_id: that.data.供应商unique_id,
      //     pagination: {
      ...
      //真正的查数据
      filter_supplier_quotations(parameter, that).then(res => {
        var list = res.data;
        if (!list || list.length == 0) {
          ...
          that.setData({
            suplierList: that.data.suplierList.concat(list),
          });
          resolve(res.msg)
        }
      })
  ...
  callbackAfterDeletion(e) {
    ...
    var index = that.data.suplierList.findIndex(item => {
      return item.unique_id === info.unique_id
    })
    if (index !== -1) {
      that.data.suplierList.splice(index, 1)
      that.setData({
        suplierList: that.data.suplierList
      })
    }
  },
  ...
  onLoad: function (options) {
    let that = this
    console.log(options)
    var data = JSON.parse(options.supplierinfodata)
    that.setData({
      供应商unique_id: options.uniqueid,
      supplierinfodata: data
    })
  ...
  onShow: async function (options) {
    let that = this;
    // 代表是从其他页面跳回来的 需要把suplierList 还原为原来的数据
    that.data.suplierList = []
    var b = 1
